...
describe('Browser', () => {
  describe('isBrowser', () => {
    it('should return true', () => {
      const result = isoCrypto.isBrowser();
  ...
  describe('RandomBytes', () => {
    ...
    beforeEach(async () => {
      length = 32;
      key = isoCrypto.browserRandomBytes(length);
    });
  ...
  describe('AES', () => {
    ...
    it('should import key from buffer successfully', async () => {
      const result = await isoCrypto.browserImportKey(key);
      expect(result).toBeTruthy();
    });
  ...
  describe('SHA2', () => {
    describe('SHA256', () => {
      ...
      it('should hash buffer sucessfully', async () => {
        ...
        const output = await isoCrypto.browserSha256(input);
    ...
    describe('SHA512', () => {
      ...
      it('should hash buffer sucessfully', async () => {
        const input = utf8ToArray(TEST_MESSAGE_STR);
        const output = await isoCrypto.browserSha512(input);
        expect(output).toEqual(expectedOutput);
      });
  ...
  describe('HMAC', () => {
    ...
    beforeEach(async () => {
      output = await isoCrypto.browserHmacSha256Sign(macKey, dataToMac);
    });
